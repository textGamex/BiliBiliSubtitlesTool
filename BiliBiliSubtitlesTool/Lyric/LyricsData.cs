...
{
    public class LyricsData : ICollection<(TimeTag, string)>, ICloneable
    {
        /// <summary>
        /// 结束标志
        /// </summary>
        public const string END_FLAG = "-----END-----";
        private readonly LinkedList<(TimeTag timeTag, string lyrics)> _data =
        ...
        public LyricsData(IEnumerable<string> lrcFileRawData)
        {
        ...
        private static (TimeTag timeTag, string lyrics) GetTimeTagAndLyrics(string rawLine)
        {
            ...
            lineData.lyrics = Tools.GetLineLyric(rawLine);
            return lineData;
        }
        ...
        //{
        //    List<string> lyrics = new List<string>(_data.Count);
        ...
        public void RemoveAfter(in TimeTag removeTimeTag)
        {
            for (var node = _data.First; node != null; node = node.Next)
            {
                ...
                {
                    _data.AddBefore(node, (removeTimeTag, END_FLAG));
                    //删除node及所有node前的数据
        ...
        {
            List<string> list = new List<string>(_data.Count);
        ...
        object ICloneable.Clone()
        {
            ...
            var clone = new LyricsData(array);
            return clone;
        }

        private LyricsData((TimeTag, string)[] datas)
        {
